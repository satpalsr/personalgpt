...
import { getContextText, saveContextText, saveSources } from '@/utils/localStorage'
...
const getStreamedResponse = async (
  ...
  // if (videos) {
  //   streamedResponse = 'Based on youtube videos: '
  //   videos.forEach((video: any, index: number) => {
  //     streamedResponse += `\n ${index + 1}. [${video.title}](https://www.youtube.com/watch?v=${video.id})`
  //   })
  //   streamedResponse += '\n I'
  //   console.log(`videos are there`)
  // } else {
  //   console.log(`videos are null`)
  // }
...
export function useChat({
  ...
  async function triggerRequest(chatRequest: ChatRequest) {
    ...
    try {
      ...
      //@ts-ignore
      const youtubeSearch = extraMetadataRef.current.body?.youtubeSearch
      // console.log(`value of ff youtubeSearch: ${youtubeSearch}`)

      let videos;

      // console.log(`youtubeSearch is ${youtubeSearch}`)

      let contextText: string | null = await getContextText(chatId);

      if (youtubeSearch) {

          // console.log(`chatid is ${chatId}`)
          // console.log(`Read from localstorage: the context text is ${contextText}`)

          if (!contextText) {

            const searchQuery = chatRequest.messages[0].content
            ...
            sources = videos.map((video: any) => ({
              id: video.id,
              title: video.title,
              thumbnail: video.thumbnail
            }));

            await saveSources(chatId, sources)

            // console.log(`sources set ${JSON.stringify(sources)}`)
            
            contextText = videos.map((video: any) => `video title: ${video.title}, video transcript: ${video.transcript}`).join('\n');
            // console.log(`contextText is ${contextText}`)



            // Save it to localstorage
            await saveContextText(chatId, contextText)
            // console.log(`saved context text`)
          } else {
            // console.log(`contexText not null`)
          }
    
      }

      const updatedBody = { ...extraMetadataRef.current.body, contextText: contextText};
  ...
  // console.log(`sources before returnset ${JSON.stringify(sources)}`)
